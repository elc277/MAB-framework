...
def main(
    ...
    if len(unique_agents) == 1:
        agent_text = f" ({int(unique_agents[0])} agents, multiple ratios)"
    else:
        agent_text = " (multiple agent counts, multiple ratios)"
    ...
    summary["ratio_label"] = summary["arms_per_agent"].apply(lambda x: f"1:{int(x)}")
...
if __name__ == "__main__":
    main(
        csv_path=r"results/big_experiment_100_agents_multi_ratio/big_experiment_100_agents_multi_ratio.csv",
        out_dir=r"results/big_experiment_100_agents_multi_ratio/plots"
    )