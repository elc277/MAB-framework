...
def main(csv_path, out_dir=None):
    ...
    required_columns = [
        "n_agents", "n_arms", "arms_per_agent", "epsilon",
        "group_total_reward", "mean_agent_reward",
        "collision_rate", "reward_inequality",
    ]
    ...
    if "degradation_strength" in df.columns:
        strengths = sorted(df["degradation_strength"].dropna().unique())
        if len(strengths) == 1:
            policy_text += f" (strength={strengths[0]})"
...
if __name__ == "__main__":
    main(
        csv_path=r"results/big_experiment_100_agents_degradation_collision/big_experiment_100_agents_degradation_collision.csv",
        out_dir=r"results/big_experiment_100_agents_degradation_collision/plots"
    )